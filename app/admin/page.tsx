...
export default function AdminPage() {
  ...
  if (status === 'authenticated' && session.user?.role !== 'admin') {
    return (
      ...
      >
        <div
          className="w-full max-sm:max-w-xs max-w-md space-y-8 rounded-lg p-8 max-sm:p-6 shadow-lg"
          style={{
        ...
        >
          <div className="text-center">
            ...
            <h2 className="mt-4 text-2xl font-bold" style={{ color: 'var(--color-fg-bold)' }}>
              Access Denied
            </h2>
            <p className="mt-2 text-sm" style={{ color: 'var(--color-fg-light)' }}>
              You are signed in as <strong>{session.user?.email}</strong>, but you don't have admin
              privileges.
            </p>
          </div>
